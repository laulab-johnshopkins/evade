...
def test_psi(dihedral_arr_index, res_index, expected_vals):
    dihedral_df = evade.dihedrals.get_dihedrals_for_resindex_list(u.residues.resindices[0:3], u, stop=3)
...
def test_chi1_sort_by_dihedrals(dihedral_arr_index, res_index, expected_vals):
    dihedral_df = evade.dihedrals.get_dihedrals_for_resindex_list(u.residues.resindices[0:3], u, stop=3, sort_by="dihedral")
    all_dihedrals = dihedral_df.to_numpy()
    all_dihedral_labels = dihedral_df.index.tolist()
...
def test_circ_corr(dihedral_arr_index, expected_vals):
    ...
    all_dihedrals = dihedral_df.to_numpy()
    all_dihedral_labels = dihedral_df.index.tolist()
    score_df = evade.dihedrals.get_dihedral_score_matrix(dihedral_df, "circ_corr")
...
def test_mut_inf(dihedral_arr_index, expected_vals):
    dihedral_df = evade.dihedrals.get_dihedrals_for_resindex_list(u.residues.resindices[0:3], u)
    all_dihedrals = dihedral_df.to_numpy()
    all_dihedral_labels = dihedral_df.index.tolist()

    score_df = evade.dihedrals.get_dihedral_score_matrix(dihedral_df, "mut_inf")
    score_matrix = score_df.to_numpy()
    np.testing.assert_allclose(score_matrix[dihedral_arr_index], expected_vals, rtol=1e-5)